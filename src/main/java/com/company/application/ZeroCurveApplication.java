...
public class ZeroCurveApplication extends Application {
    ...
    private static void zeroCurve(ModelType modelType, String curveString) throws InterpolationException, IOException {
         ...
         */
        RatesCurve fullSwapCurve;
        if (dataPointList.stream().anyMatch(dataPoint -> dataPoint.getType().equals(RateType.SWAP))) {
            ...
            fullSwapCurve = RatesCurveUtils.getFullSwapsCurveAfterInterpolatingMissingMaturities(swapsRatesCurve, swapInterpolator);
    ...
    public void start(Stage primaryStage) throws Exception {
        ...
        choiceBox.getItems().addAll("GBP Zero Curve", "CHF Zero Curve", "ZAR Zero Curve", "GBP Adj. Zero Curve", "TestSet1");
        choiceBox.setValue("");
        //Model Choice Box
        ...
        fwdChartRateStartDatePicker.valueProperty().addListener((observable, oldValue, newValue) -> {
        ...
        this.fwdChartRateEndDatePicker.valueProperty().addListener((observable, oldValue, newValue) -> {
            getForwardCurveLineChart();
        });

        Label fwdStartRateLabel = new Label("Enter Forward Start Date: ");
        this.fwdRateStartDatePicker.setValue(startDate.plusDays(2));
        this.fwdRateStartDatePicker.valueProperty().addListener((observable, oldValue, newValue) -> {
        ...
        this.fwdRateEndDatePicker.valueProperty().addListener((observable, oldValue, newValue) -> {
            updateFwoDateSelected();
        });
    ...
    private void updateChartAndDisplayedData(Label labelChoiceBox, String choiceBoxValue, String modeledChoiceBoxValue) {
        ...
        try {
            ...
            updateDateSelected();
            updateFwoDateSelected();
            if(interpolator instanceof MonotoneConvex){
                printDiscreteForwards(((MonotoneConvex)interpolator).getDiscreteForwardRates());
            }
        } catch (InterpolationException e) {
            e.printStackTrace();
        } catch (IOException e) {
    ...
    private static void printDiscreteForwards(double[] discreteForwardRates){
        for (int i = 0; i < discreteForwardRates.length; i++) {
            System.out.println("Time Bucket: " + i + " Discrete Fwd Rate: " + String.format("%.04f", 100 * discreteForwardRates[i]) + "%");
        }
    }
}
